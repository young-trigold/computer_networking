...
电信公司已经对所谓的第四代（4G）无线技术进行了巨大的投资，该技术提供了高达 60Mbps 的实际下载速度。但更高速的广域接入技术--第五代（5G）广域无线网络：已经在部署中。我们将在第七章中介绍无线网络和移动性的基本原理，以及 WiFi、4G 和 5G 技术（以及更多！）。
...
现在考虑由当前 Internet 所引发的另一个例子。图 1-20a 显示了与一个计算机网络相连的两个端系统：一台服务器和一个客户。考虑从服务器向客户传送一个文件的吞吐量。服务器以速率为 Rs 的接入链路与网络相连，且客户以速率为 Rc 的接入链路与网络相连。现在假定在通信网络核心中的所有链路具有非常高的传输速率，即该速率比 Rs 和 Rc 要高得多。目前 Internet 的核心的确超量配置了高速率的链路，从而很少出现拥塞。同时假定在整个网络中发送的比特都是从该服务器到该客户。在这个例子中，因为计算机网络的核心就像一个粗大的管子，所以比特从源向目的地的流动速率仍是 Rs 和 Rc 中的最小者，即吞吐量 = min {Rs, Rc}。因此，在今天 Internet 中对吞吐量的限制因素通常是接入网。
...
对于今天的许多机构（包括大大小小的公司、大学和政府机关）而言，Internet 已经成为与其使命密切相关的一部分了。许多人也依赖 Internet 从事各种职业、社会和个人活动。目前，数以亿计的物品（包括可穿戴设备和家用设备）与 Internet 相连。但是在所有这一切背后，存在着一个阴暗面，其中的“坏家伙”试图对我们的日常生活进行破坏，如损坏我们与 Internet 相连的计算机，侵犯我们的隐私以及使我们依赖的 Internet 服务无法运行。
...
因为我们在计算机网络和 Internet 协议方面还没有专业知识，所以这里我们将从审视某 些今天最为流行的与安全性相关的问题开始。这将刺激我们的胃口，以便我们在后续章节中进行更为充实的讨论。我们在这里以提岀问题开始：什么会出现问题？计算机网络是如何受到攻击的？今天一些最为流行的攻击类型是什么？

### 1.6.1. 恶意软件

因为我们要从/向 Internet 接收/发送数据，所以我们将设备与 Internet 相连。这包括各种好东西，例如 Instagram 帖子、Internet 搜索结果、流式音乐、视频会议、流式电影等。但不幸的是，伴随好的东西而来的还有恶意的东西，这些恶意的东西可统称为 **恶意软件(malware)**，它们能够进入并感染我们的设备。一旦恶意软件感染我们的设备，就能够做各种不正当的事情，包括删除我们的文件，安装间谍软件来收集我们的隐私信息，如身份证号、口令和击键，然后将这些（当然经 Internet）发送给坏家伙。我们的受害主机也可能成为数以千计的类似受害设备网络中的一员，它们被统称为 **僵尸网络(botnet)**，坏家伙利用僵尸网络控制并有效地对目标主机展开垃圾邮件分发或分布式拒绝服务攻击（很快将讨论）。

至今为止的多数恶意软件是 **自我复制(self-replicating)** 的：一旦它感染了一台主机，就会从那台主机寻求进人 Internet 上的其他主机，从而形成新的感染主机，再寻求进入更多的主机。以这种方式，自我复制的恶意软件能够指数式地快速扩散。恶意软件能够以病毒或蠕虫的形式扩散。**病毒(virus)** 是一种需要某种形式的用户交互来感染用户设备的恶意软件。典型的例子是包含恶意可执行代码的电子邮件附件。如果用户接收并打开这样的附件，不经意间就在其设备上运行了该恶意软件。通常，这种电子邮件病毒是自我复制的: 例如，一旦执行，该病毒可能向用户地址簿上的每个接收方发送一个具有相同恶意附件的相同报文。**蠕虫(worm)** 是一种无须任何明显用户交互就能进入设备的恶意软件。例如，用户也许运行了一个攻击者能够发送恶意软件的脆弱网络应用程序。在某些情况下，没有用户的任何干预，该应用程序可能从 Internet 接收恶意软件并运行它，生成了蠕虫。新近感染设备中的蠕虫则能扫描 Internet，搜索其他运行相同网络应用程序的易受感染的主机。当它发现其他易受感染的主机时，便向这些主机发送一个它自身的副本。今天，恶意软件无所不在且防范成本高。当你用这本书学习时，我们鼓励你思考下列问题：计算机网络设计者能够采取什么防御措施，以使与 Internet 连接的设备免受恶意软件的攻击？

### 1.6.2. 拒绝服务(DoS)

另一种宽泛类型的安全性威胁称为 **拒绝服务攻击(Denial-of-Service (DoS) attack)**。 顾名思义，DoS 攻击使得网络、主机或其他基础设施部分不能由合法用户使用。Web 服务器、电子邮件服务器、DNS 服务器（在第 2 章中讨论）和机构网络都能够成为 DoS 攻击的目标。Internet DoS 攻击极为常见，每年会出现数以千计的 DoS 攻击［Moore 2001］。访问数字攻击图（Digital Attack Map）站点可以可视化了世界范围内每天最厉害的 DoS 攻击［DAM 2020］。大多数 Internet DoS 攻击属于下列三种类型之一：
...
今天的许多用户经无线设备接入 Internet，如 WiFi 连接的膝上计算机或使用蜂窝 Internet 连接的手持设备(在第 7 章中讨论)。无所不在的 Internet 接入极为便利并让移动用户方便地使用令人惊奇的新应用程序的同时，也产生了严重的安全脆弱性：在无线传输设备的附近放置一台被动的接收机，该接收机就能得到传输的每个分组的副本！这些分组包含了各种敏感信息，包括口令、身份证号、商业秘密和隐秘的个人信息。记录每个流经的分组副本的被动接收机被称为 **分组嗅探器(packet sniffer)**。

嗅探器也能够部署在有线环境中。在有线的广播环境中，如在许多以太网 LAN 中，分组嗅探器能够获得经该 LAN 发送的所有分组。如在前面描述的那样，电缆接入技术也广播分组，因此易于受到嗅探攻击。此外，获得某机构与 Internet 连接的接入路由器或接入链路访问权的坏家伙能够放置一台嗅探器以产生从该机构出入的每个分组的副本，再对嗅探到的分组进行离线分析，就能得出敏感信息。
...
生成具有任意源地址、分组内容和目的地址的分组，然后将这个人工制作的分组传输到 Internet 中，Internet 将忠实地将该分组转发到目的地，这一切都极为容易（当你学完这本教科书后，你将很快具有这方面的知识了！）。想象某个接收到这样一个分组的不会猜疑的 接收方（比如说一台 Internet 路由器），将该（虚假的）源地址作为真实的，进而执行某些嵌入在该分组内容中的命令（比如说修改它的转发表）。将具有虚假源地址的分组注入 Internet 的能力被称为 **IP 哄骗(IP spoofing)**，而它只是一个用户能够冒充另一个用户的许多方式中的一种。

为了解决这个问题，我们需要采用端点鉴别，即一种使我们能够确信一个报文源自我们认为它应当来自的地方的机制。当你继续学习本书各章时，再次建议你思考怎样为网络应用程序和协议做这件事。我们将在第 8 章探讨端点鉴别机制。

在本节结束时，值得思考一下 Internet 是如何从一开始就落入这样一种不安全的境地的。大体上讲，答案是：Internet 最初就是基于“一群相互信任的用户连接到一个透明的网 络上”这样的模型［Blumenthal 2001］进行设计的，在这样的模型中，安全性是没有必要的。初始的 Internet 体系结构在许多方面都深刻地反映了这种相互信任的理念。例如，一个用户向任何其他用户发送分组的能力是默认的，而不是一种请求/准予的能力，还有用户身份取自所宣称的表面价值，而不是默认地需要鉴别。

但是今天的 Internet 无疑并不涉及“相互信任的用户”。但是，今天的用户仍然需要通信，当他们不必相互信任时，他们也许希望匿名通信，也许间接地通过第三方通信（例如我们将在第 2 章中学习的 Web 代理，我们将在第 7 章学习的移动性协助代理），也许不信任他们通信时使用的硬件、软件甚至空气。随着我们进一步学习本书，会面临许多安全性相关的挑战：我们应当寻求对嗅探、端点假冒、中间人攻击、DDoS 攻击、恶意软件等的防护办法。我们应当记住：在相互信任的用户之间的通信是一种例外而不是规则。欢迎你到现代计算机网络世界！

## 1.7. 计算机网络的历史

第 1-1 至 1-6 节介绍了计算机网络和互联网技术的概况。你现在应该知道的足够让你的家人和朋友印象深刻了。然而，如果你真的想在下一个鸡尾酒会上大放异彩，你应该在你的论述中洒下关于互联网迷人历史的花絮[Segaller 1998]。

### 1.7.1. 分组交换的发展：1961-1972

计算机网络领域和今天的互联网可以追溯到 20 世纪 60 年代初，当时电话网络是世界上最主要的通信网络。回顾第 1.3 节，电话网络使用电路交换将信息从发送方传输到接收方--鉴于语音在发送方和接收方之间以恒定的速率传输，这是一个合适的选择。鉴于 20 世纪 60 年代初计算机的重要性不断增加，以及分时计算机的出现，也许很自然地要考虑如何将计算机连接起来，以便在地理上分布的用户之间共享。这些用户产生的流量可能是突发性的活动，如向远程计算机发送命令，然后在等待答复或考虑收到的答复时处于不活动状态。

世界各地的三个研究小组开始发明分组交换，作为电路交换的有效和稳健的替代方案，每个小组都不知道其他人的工作[Leiner 1998]。第一个发表的关于分组交换技术的工作是 Leonard Kleinrock [Kleinrock 1961; Kleinrock 1964]，他当时是麻省理工学院的研究生。Kleinrock 的工作使用排队理论，优雅地证明了分组交换方法对突发流量源的有效性。1964 年，兰德研究所的保罗-巴兰[Baran 1964]已经开始研究利用分组交换在军事网络上实现安全语音，而在英国的国家物理实验室，唐纳德-戴维斯和罗杰-斯坎特伯里也在发展他们关于分组交换的想法。

麻省理工学院、兰德公司和国家物理实验室的工作为今天的互联网奠定了基础。但是，互联网也有一个漫长的历史，那就是让我们来建造它并演示它的态度，这也可以追溯到 20 世纪 60 年代。J.C.R.Licklider[1990 12 月]和 Lawrence Roberts 都是 Kleinrock 在麻省理工学院的同事，后来在美国的高级研究计划局（ARPA）领导计算机科学项目。罗伯茨发表了 ARPA 网络的总体计划[Roberts 1967]，这是第一个分组交换的计算机网络，是今天公共互联网的直接祖先。1969 年的劳动节，在 Kleinrock 的监督下，第一台分组交换机安装在加州大学洛杉矶分校，随后不久又有三台分组交换机安装在斯坦福研究所（SRI）、加州大学圣巴巴拉分校和犹他大学（图 1.26）。到 1969 年底，互联网的雏形已经有四个节点大了。Kleinrock 回忆说，第一次使用该网络时，从加州大学洛杉矶分校到 SRI 进行了一次远程登录，使系统崩溃[Kleinrock 2004]。

到 1972 年，ARPA 网络已经发展到大约 15 个节点，并由罗伯特-卡恩进行了首次公开演示。ARPAnet 终端系统之间的第一个主机到主机协议，即网络控制协议（NCP），已经完成[RFC 001]。有了端到端的协议，现在可以编写应用程序了。雷-汤姆林森在 1972 年编写了第一个电子邮件程序。

### 1.7.2. 专有网络和国际网络：1972-1980

最初的 ARPAnet 是一个单一的、封闭的网络。为了与 ARPAnet 主机通信，人们必须实际连接到另一个 ARPAnet IMP。在 20 世纪 70 年代早期到中期，除了 ARPAnet 之外，还出现了其他独立的分组交换网络。ALOHANet，一个连接夏威夷岛屿上的大学的微波网络[Abramson 1970]，以及 DARPA 的分组卫星[RFC 829]和分组无线电网络[Kahn 1978]；Telenet，一个基于 ARPAnet 技术的 BBN 商业分组交换网络。Cyclades，由 Louis Pouzin 开创的法国分组交换网络[Think 2012]；20 世纪 60 年代末和 70 年代初的 Tymnet 和 GE 信息服务网络等时间共享网络[Schwartz 1977]；IBM 的 SNA（1969-1974），它与 ARPAnet 的工作并行[Schwartz 1977]。

网络的数量在不断增加。事后我们可以看到，开发一个将网络连接在一起的全面架构的时机已经成熟了。文顿-瑟夫和罗伯特-卡恩[Cerf 1974]在网络互连方面做了开创性的工作（在国防高级研究计划局（DARPA）的赞助下），实质上是创建了一个网络的网络；术语 internetting 是用来描述这项工作的。

这些架构原则在 TCP 中得到了体现。然而，早期版本的 TCP 与今天的 TCP 有很大不同。早期版本的 TCP 通过终端系统的重传（仍然是今天 TCP 的一部分）将可靠的按顺序交付数据与转发功能（今天由 IP 执行）相结合。早期对 TCP 的实验，结合对不可靠的、非流量控制的、端到端的传输服务对应用的重要性的认识
导致了 IP 从 TCP 中分离出来，并发展了 UDP 协议。我们今天看到的三个关键的互联网协议--TCP、UDP 和 IP--在 20 世纪 70 年代末就已经概念化了。

除了 DARPA 与互联网有关的研究，许多其他重要的网络活动也在进行中。在夏威夷，Norman Abramson 正在开发 ALOHAnet，这是一个基于数据包的无线电网络，允许夏威夷群岛上的多个远程站点相互通信。ALOHA 协议[Abramson 1970]是第一个多址协议，允许地理上分布的用户共享一个单一的广播通信介质（一个无线电频率）。Metcalfe 和 Boggs 在 Abramson 的多址协议工作的基础上，为基于电线的共享广播网络开发了以太网协议[Metcalfe 1976]。有趣的是，梅特卡夫和博格斯的以太网协议是出于连接多台 PC、打印机和共享磁盘的需要[Perkins 1994]。25 年前，远在 PC 革命和网络爆炸之前，梅特卡夫和博格斯就为今天的 PC 局域网奠定了基础。

### 1.7.3. 网络的激增：1980-1990

到 20 世纪 70 年代末，大约有两百台主机连接到 ARPA 网络。到 20 世纪 80 年代末，连接到公共互联网的主机数量将达到十万台，这是一个看起来很像今天的互联网的网络联盟。20 世纪 80 年代将是一个巨大的增长时期。

这种增长的大部分原因是为创建连接大学的计算机网络所做的一些独特努力。BITNET 在东北地区的几所大学之间提供电子邮件和文件传输。CSNET（计算机科学网络）的形成是为了连接那些没有机会进入 ARPA 网络的大学研究人员。1986 年，NSFNET 成立，提供对 NSF 赞助的超级计算中心的访问。从最初的 56kbps 的骨干网速度开始，NSFNET 的骨干网在十年结束时将以 1.5Mbps 的速度运行，并将作为连接区域网络的主要骨干网。

在 ARPAnet 社区，今天的互联网架构的许多最后部分正在逐步到位。1983 年 1 月 1 日，TCP/IP 被正式部署为 ARPAnet 的新标准主机协议（取代了 NCP 协议）。从 NCP 到 TCP/IP 的过渡[RFC 801]是一个标志日事件--所有的主机都被要求从那一天起转移到 TCP/IP。在 20 世纪 80 年代末，对 TCP 进行了重要的扩展，以实现基于主机的拥堵控制[Jacobson 1988]。DNS 也被开发出来，用于映射人类可读的互联网名称（例如，gaia.cs.umass.edu）和其 32 位 IP 地址[RFC 1034]。

在 ARPA 网络发展的同时（大部分是美国的努力），在 20 世纪 80 年代初，法国启动了 Minitel 项目，这是一个雄心勃勃的计划，将数据网络带入每个人的家庭。在法国政府的赞助下，Minitel 系统包括一个公共分组交换网络（基于 X.25 协议套件）、Minitel 服务器和内置低速调制解调器的廉价终端。1984 年，法国政府向每个想要 Minitel 终端的法国家庭免费赠送了一个 Minitel 终端，Minitel 获得了巨大的成功。Minitel 网站包括免费网站--如电话簿网站--以及私人网站，这些网站向每个用户收取按使用量计算的费用。在 20 世纪 90 年代中期的高峰期，它提供了超过 20,000 项服务，从家庭银行到专业研究数据库。在大多数美国人听说过互联网的十年前，Minitel 就已经进入了大部分法国家庭。

### 1.7.4. 互联网的爆炸性增长：20 世纪 90 年代

20 世纪 90 年代迎来了一系列事件，象征着互联网的持续发展和即将到来的商业化。ARPAnet，互联网的鼻祖，已经不复存在。1991 年，NSFNET 取消了对 NSFNET 用于商业目的的限制。NSFNET 本身将于 1995 年退役，互联网骨干流量由商业互联网服务提供商承担。

20 世纪 90 年代的主要事件是万维网应用的出现，它将互联网带入全世界数百万人的家庭和企业。万维网作为一个平台，促成并部署了数百种我们今天认为理所当然的新应用，包括搜索（如谷歌和必应）互联网商务（如亚马逊和 eBay）和社交网络（如 Facebook）。

网络是由 Tim Berners-Lee 在 1989 年至 1991 年间在欧洲核子研究中心发明的[Berners-Lee 1989]，其基础是源于 1940 年代 Vannevar Bush[Bush 1945]和 1960 年代以来 Ted Nelson[Xanadu 2012]在超文本方面的早期工作的想法。伯纳斯-李和他的同事们开发了最初版本的 HTML、HTTP、网络服务器和浏览器--网络的四个关键组成部分。大约在 1993 年底，大约有两百个网络服务器在运行，这些服务器的集合只是即将到来的一个预兆。大约在这个时候，一些研究人员正在开发带有 GUI 界面的网络浏览器，包括 Marc Andreessen，他和 Jim Clark 一起成立了 Mosaic 通信公司，后来成为 Netscape 通信公司[Cusumano 1998; Quittner 1998]。到 1995 年，大学生们每天都在使用网景浏览器浏览网页。大约在这个时候，大大小小的公司开始运营网络服务器，并通过网络进行商业交易。1996 年，微软开始生产浏览器，这引发了网景和微软之间的浏览器战争，几年后微软赢得了这场战争[Cusumano 1998]。

20 世纪 90 年代后半期是互联网巨大增长和创新的时期，大公司和数以千计的初创公司都在创造互联网产品和服务。到了千禧年末，互联网已经支持了数百种流行的应用，包括四种非常流行的应用：

- 电子邮件，包括附件和可通过网络访问的电子邮件
- 网络，包括网络浏览和互联网商务
- 即时通讯，包括联系人名单
- 由 Napster 公司开创的 MP3 点对点文件共享。

有趣的是，前两个杀手级应用来自研究界，而后两个则是由一些年轻的企业家创造的。

1995 年至 2001 年期间，互联网在金融市场上的发展如坐过山车。在盈利之前，数以百计的互联网初创公司进行了首次公开募股，并开始在股票市场进行交易。许多公司在没有任何重要收入来源的情况下，估值就达到了数十亿美元。互联网股票在 2000-2001 年崩溃，许多创业公司关闭。然而，一些公司成为了互联网领域的大赢家，包括微软、思科、雅虎、eBay、谷歌和亚马逊。

### 1.7.5. 新千禧年

在 21 世纪的前 20 年里，也许没有其他技术比互联网连同与互联网连接的智能手机对社会的改变更大。而计算机网络的创新仍在继续，速度很快。各个方面都在取得进展，包括在接入网和网络主干网中部署更快的路由器和更高的传输速度。但以下的发展值得特别关注。

- 自本世纪初以来，我们一直看到宽带互联网接入家庭的积极部署：不仅是电缆调制解调器和 DSL，还有光纤到户，以及现在的 5G 固定无线。这种高速互联网接入为丰富的视频应用创造了条件，包括传播用户生成的视频（例如 YouTube），按需播放电影和电视节目（例如 Netflix），以及多人视频会议（例如 Skype、Facetime 和 Google Hangouts）。

- 高速无线互联网接入的日益普及，不仅使人们在移动中不断保持联系成为可能，而且还使新的特定地点的应用成为可能，如 Yelp、Tinder 和 Waz。2011 年，连接到互联网的无线设备的数量超过了有线设备的数量。这种高速无线接入为手持电脑（iPhone、安卓、iPad 等）的迅速出现创造了条件，它们可以不受限制地持续接入互联网。

- 在线社交网络：如 Facebook、Instagram、Twitter 和微信（在中国大受欢迎）——在互联网之上创造了巨大的人际网络。这些社交网络中的许多都被广泛用于信息传递和照片分享。今天，许多互联网用户主要 "生活 "在一个或多个社交网络中。通过他们的 API，在线社交网络为新的网络化应用创造了平台，包括移动支付和分布式游戏。

- 在线服务提供商，如谷歌和微软，已经部署了他们自己的广泛的私人网络，这些网络不仅将他们分布在全球的数据中心连接在一起，而且被用来绕过互联网。这些网络不仅将其分布在全球的数据中心连接在一起，而且通过直接与较低级别的 ISP 对接，尽可能地绕过互联网。因此，谷歌提供的搜索结果和电子邮件访问几乎是即时的，就像他们的数据中心在一个人的电脑中运行一样。

- 许多互联网商业公司现在都在 "云 "中运行他们的应用程序，如亚马逊的 EC2，微软的 Azure，或阿里巴巴的云。许多公司和大学也已将他们的互联网应用（如电子邮件和网络托管）迁移到云中。云计算公司不仅为应用程序提供可扩展的计算和存储环境，而且还为应用程序提供隐式访问其高性能的私有网络。
  网络。
